use cosmwasm_std::{
    callable_point, dynamic_link, entry_point, from_slice, to_binary, to_vec, Addr, Binary,
...
fn handle_number(deps: Deps) -> Result<i32, ContractError> {
    ...
    // let response = Response::default().add_attribute("value_by_dyn", value_dyn.to_string());
